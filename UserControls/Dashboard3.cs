...
namespace Rheometer_Torque.UserControls
{
    public partial class Dashboard3 : UserControl
    {
        ...
        private void Dashboard3_Paint(object sender, PaintEventArgs e)
        {
            //清除
            ...
            //绘制表盘刻度  使用线条
            var scaleColor = Color.GreenYellow;
            ...
            var pointerBrush = new SolidBrush(scaleColor);
            var pointerPen = new Pen(scaleColor);
            ...
            //绘制指针2
            {
                e.Graphics.FillEllipse(Brushes.GreenYellow, X0 - 6, Y0 - 6, 12, 12);
                var R5 = R0 - 45;
                var jiaodu = value / 360f * Math.PI;
                var X1 = X0 + R5 * Math.Cos(jiaodu);
                var Y1 = Y0 + R5 * Math.Sin(jiaodu);
                e.Graphics.DrawLine(new Pen(Color.GreenYellow, 3), new PointF(X0, Y0), new PointF((float)X1, (float)Y1));
            }
